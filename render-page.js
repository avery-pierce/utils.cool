...
var _react = _interopRequireDefault(__webpack_require__(836));
...
var React = _interopRequireWildcard(__webpack_require__(836));
...
var _react = __webpack_require__(836);
...
const preferDefault=m=>m&&m.default||m;exports.components={"component---src-pages-index-js":preferDefault(__webpack_require__(595)),"component---src-pages-timer-js":preferDefault(__webpack_require__(563))};
...
/* harmony export */ });
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(836);
...
const trimmedPathname=stripPrefix(pathname,decodeURIComponent("/utils.cool"))// Remove any hashfragment
...
// EXTERNAL MODULE: external "/Users/averypierce/Projects/utils-cool/node_modules/react/index.js"
var index_js_ = __webpack_require__(836);
...
if(this.staticQueryDb[staticQueryHash]){const jsonPayload=this.staticQueryDb[staticQueryHash];return{staticQueryHash,jsonPayload};}return this.memoizedGet(`${"/utils.cool"}/page-data/sq/d/${staticQueryHash}.json`).then(req=>{const jsonPayload=JSON.parse(req.responseText);return{staticQueryHash,jsonPayload};}).catch(()=>{throw new Error(`We couldn't load "${"/utils.cool"}/page-data/sq/d/${staticQueryHash}.json"`);});})).then(staticQueryResults=>{const staticQueryResultsMap={};staticQueryResults.forEach(({staticQueryHash,jsonPayload})=>{staticQueryResultsMap[staticQueryHash]=jsonPayload;this.staticQueryDb[staticQueryHash]=jsonPayload;});return staticQueryResultsMap;});return Promise.all([componentChunkPromise,staticQueryBatchPromise]).then(([pageResources,staticQueryResults])=>{let payload;if(pageResources){payload={...pageResources,staticQueryResults};finalResult.payload=payload;emitter.emit(`onPostLoadPageResources`,{page:payload,pageResources:payload});}this.pageDb.set(pagePath,finalResult);if(finalResult.error){return{error:finalResult.error,status:finalResult.status};}return payload;})// when static-query fail to load we throw a better error
...
const Layout=({children})=>{return/*#__PURE__*/index_js_default().createElement((index_js_default()).Fragment,null,/*#__PURE__*/index_js_default().createElement("nav",null,/*#__PURE__*/index_js_default().createElement(gatsby_browser_entry.Link,{to:"/"},"Home"),/*#__PURE__*/index_js_default().createElement(gatsby_browser_entry.Link,{to:"/timer"},"Timer")),/*#__PURE__*/index_js_default().createElement("main",null,children));};/* harmony default export */ const components_Layout = (Layout);
;// CONCATENATED MODULE: ./src/pages/index.js
const Home=({location})=>{return/*#__PURE__*/index_js_default().createElement(components_Layout,null,/*#__PURE__*/index_js_default().createElement("h1",null,"Home Page ",/*#__PURE__*/index_js_default().createElement("span",null,location.pathname)),/*#__PURE__*/index_js_default().createElement("h2",null,"Static HTML Page"));};/* harmony default export */ const pages = (Home);
...
// EXPORTS
__webpack_require__.d(__webpack_exports__, {
  "default": () => (/* binding */ timer)
});

// EXTERNAL MODULE: external "/Users/averypierce/Projects/utils-cool/node_modules/react/index.js"
var index_js_ = __webpack_require__(836);
var index_js_default = /*#__PURE__*/__webpack_require__.n(index_js_);
;// CONCATENATED MODULE: ./src/components/CoolMobileFullscreenLayout.js
function CoolMobileFullscreenLayout({children,style}){return/*#__PURE__*/index_js_default().createElement("div",{style:{display:"flex",position:"fixed",top:0,left:0,bottom:0,right:0,flex:1,flexDirection:"column",...style}},children);}
;// CONCATENATED MODULE: ./src/hooks/useCurrentDate.js
function useCurrentDate(refreshRateHz=60){const{0:date,1:setDate}=(0,index_js_.useState)(new Date());(0,index_js_.useEffect)(()=>{const handle=setInterval(()=>{setDate(new Date());},1000/refreshRateHz);return()=>clearInterval(handle);},[refreshRateHz]);return date;}
;// CONCATENATED MODULE: ./src/components/CoolText.js
function CoolTitle({children,style}){return/*#__PURE__*/React.createElement("h1",{style:{fontFamily:"Monoton",textTransform:"uppercase",fontWeight:"normal",fontSize:"30pt",color:"#FFF",...style}},children);}function CoolText({children,style,bold}){return/*#__PURE__*/index_js_default().createElement("span",{style:{fontFamily:"Roboto",fontWeight:bold?"500":"normal",color:"#FFF",...style}},children);}
;// CONCATENATED MODULE: ./src/hooks/useLocalStorageState.js
...
function getLocalObject(key){const value=window.localStorage.getItem(key);return JSON.parse(value);}/// Returns `true` if this key has never been populated.
function isLocalObjectPopulated(key){return window.localStorage.getItem(key)!==null;}/// Set a JSON object to local storage state
function setLocalObject(key,obj){const stringified=JSON.stringify(obj);return window.localStorage.setItem(key,stringified);}/// Removes the value at this key from storage
function removeLocalObject(key){return window.localStorage.removeItem(key);}const dateSerilaizer=date=>{if(date===null){return null;}return date.getTime();};const dateDeserializer=ms=>{return new Date(ms);};const arraySerializer=elementSerializer=>{return array=>{return array.map(elementSerializer);};};const arrayDeserializer=elementDeserializer=>{return array=>{return array.map(elementDeserializer);};};
;// CONCATENATED MODULE: ./src/pages/timer.js
const Timer=()=>{const[startDate,setStartDate]=useLocalStorageState("timer.startDate",null,dateSerilaizer,dateDeserializer);const[stopDate,setStopDate]=useLocalStorageState("timer.stopDate",null,dateSerilaizer,dateDeserializer);const[laps,setLaps]=useLocalStorageState("timer.laps",[],arraySerializer(dateSerilaizer),arrayDeserializer(dateDeserializer));const elapsed=useSecondsElapsed(startDate,stopDate);const onStart=(0,index_js_.useCallback)(()=>{setStartDate(new Date());setStopDate(null);setLaps([]);},[]);const onStop=(0,index_js_.useCallback)(()=>{setStopDate(new Date());},[]);const onTapLap=(0,index_js_.useCallback)(()=>{const newLaps=[...laps,new Date()];setLaps(newLaps);},[laps]);const lapD=lapDeltas(startDate,laps);const isTimerRunning=!!startDate&&!stopDate;return/*#__PURE__*/index_js_default().createElement(CoolMobileFullscreenLayout,null,/*#__PURE__*/index_js_default().createElement("div",{style:{flexGrow:1}}),/*#__PURE__*/index_js_default().createElement("div",{style:{padding:"1em",flexShrink:2}},/*#__PURE__*/index_js_default().createElement("div",{style:{background:"rgba(233, 233, 233, 0.65)",boxShadow:"0px 4px 12px rgba(0, 0, 0, 0.2)",borderRadius:8,padding:"1em"}},lapD.map((lapTime,i)=>/*#__PURE__*/index_js_default().createElement(LapDetail,{key:lapTime.date.getTime(),index:i,durationSeconds:lapTime.duration/1000,deltaSeconds:lapTime.delta/1000,timestamp:lapTime.date})))),/*#__PURE__*/index_js_default().createElement("div",{style:{padding:"1em"}},/*#__PURE__*/index_js_default().createElement(CoolText,{bold:true,style:{display:"block",textAlign:"center",fontSize:"60pt"}},formatTimeInterval(elapsed))),/*#__PURE__*/index_js_default().createElement(StopwatchControls,{onStart:onStart,onStop:onStop,onLap:onTapLap,isTimerRunning:isTimerRunning}));};const secondFormatter=new Intl.NumberFormat("en-US",{minimumIntegerDigits:2,minimumFractionDigits:2,maximumFractionDigits:2});function formatTimeInterval(seconds){const minutes=Math.floor(seconds/60);const remainingSeconds=seconds%60;const formattedSeconds=secondFormatter.format(remainingSeconds);return`${minutes}:${formattedSeconds}`;}function StopwatchControls({onStart,onStop,onLap,isTimerRunning}){return/*#__PURE__*/index_js_default().createElement("div",{style:{display:"flex",flexDirection:"row"}},!isTimerRunning&&/*#__PURE__*/index_js_default().createElement(CoolButton,{onClick:onStart,style:{flex:1,margin:"1em",backgroundColor:"green"}},"Start"),isTimerRunning&&/*#__PURE__*/index_js_default().createElement(CoolButton,{onClick:onLap,style:{flex:1,margin:"1em",backgroundColor:"blue"}},"Lap"),/*#__PURE__*/index_js_default().createElement(CoolButton,{disabled:!isTimerRunning,onClick:onStop,style:{flex:1,margin:"1em",marginLeft:0,backgroundColor:"red"}},"Stop"));}function CoolButton({children,onClick,style,disabled}){return/*#__PURE__*/index_js_default().createElement("button",{onClick:onClick,style:{height:"7em",borderRadius:8,fontSize:"15pt",border:"none",textTransform:"uppercase",fontWeight:"bold",opacity:disabled?0.4:1.0,...style}},children);}function lapDeltas(startTime,lapTimes){const lapDurations=lapTimes.map((date,i,allTimes)=>{const previousStop=i===0?startTime:allTimes[i-1];const lapDuration=date.getTime()-previousStop.getTime();return{date,duration:lapDuration};});return lapDurations.map((duration,i,allDurations)=>{if(i===0){return{...duration,delta:null};}else{const prevDuration=allDurations[i-1];const delta=duration.duration-prevDuration.duration;return{...duration,delta};}});}function LapDetail({index,timestamp,durationSeconds,deltaSeconds}){return/*#__PURE__*/index_js_default().createElement("div",{style:{display:"flex",flexDirection:"row",margin:"0.6em auto"}},/*#__PURE__*/index_js_default().createElement("div",{style:{paddingRight:"0.4em",marginRight:"0.4em",borderRight:"solid 0.5px rgba(0, 0, 0, 0.5)"}},/*#__PURE__*/index_js_default().createElement(CoolText,{style:{color:"black"}},index+1)),/*#__PURE__*/index_js_default().createElement("div",{style:{display:"flex",flexDirection:"column"}},/*#__PURE__*/index_js_default().createElement("span",{style:{fontSize:"1.2em"}},/*#__PURE__*/index_js_default().createElement(CoolText,{style:{color:"black",fontWeight:"bold",marginRight:"0.4em"}},durationSeconds,"s"),/*#__PURE__*/index_js_default().createElement(CoolText,{style:{color:"black"}},"(",/*#__PURE__*/index_js_default().createElement(CoolText,{style:{color:deltaSeconds>0?"red":"green"}},deltaSeconds),")")),/*#__PURE__*/index_js_default().createElement(CoolText,{style:{color:"black",opacity:0.6}},timestamp.toLocaleString())));}function useSecondsElapsed(startDate,stopDate){const currentDate=useCurrentDate(30);if(!startDate){return 0;}const compareDate=stopDate||currentDate;const ms=compareDate.getTime()-startDate.getTime();return ms/1000;}/* harmony default export */ const timer = (Timer);
...
"use strict";
module.exports = require("/Users/averypierce/Projects/utils-cool/.cache/ssr-builtin-trackers/fs");
...
"use strict";
module.exports = require("/Users/averypierce/Projects/utils-cool/node_modules/react-dom/server.js");
...
"use strict";
module.exports = require("/Users/averypierce/Projects/utils-cool/node_modules/react/index.js");
...
"use strict";
module.exports = JSON.parse('{"polyfill":["/polyfill-3fd9aed4207c4e72e4c6.js"],"app":["/app-01d3d47974558a5d236c.js"],"component---src-pages-index-js":["/component---src-pages-index-js-ccfe0283b5037be0df72.js"],"component---src-pages-timer-js":["/component---src-pages-timer-js-1597ea2ff9e48f586a6e.js"]}');
...
const scriptChunkMapping=`/*<![CDATA[*/window.___chunkMapping=${JSON.stringify(chunkMapping)};/*]]>*/`;postBodyComponents.push(/*#__PURE__*/React.createElement("script",{key:`chunk-mapping`,id:`gatsby-chunk-mapping`,dangerouslySetInnerHTML:{__html:scriptChunkMapping}}));let bodyScripts=[];if(chunkMapping[`polyfill`]){chunkMapping[`polyfill`].forEach(script=>{const scriptPath=`${"/utils.cool"}${script}`;bodyScripts.push(/*#__PURE__*/React.createElement("script",{key:scriptPath,src:scriptPath,noModule:true}));});}// Filter out prefetched bundles as adding them as a script tag
// would force high priority fetching.
bodyScripts=bodyScripts.concat(scripts.filter(s=>s.rel!==`prefetch`).map(s=>{const scriptPath=`${"/utils.cool"}/${JSON.stringify(s.name).slice(1,-1)}`;return/*#__PURE__*/React.createElement("script",{key:scriptPath,src:scriptPath,async:true});}));postBodyComponents.push(...bodyScripts);apiRunner(`onPreRenderHTML`,{getHeadComponents,replaceHeadComponents,getPreBodyComponents,replacePreBodyComponents,getPostBodyComponents,replacePostBodyComponents,pathname:pagePath,pathPrefix:"/utils.cool"});const html=`<!DOCTYPE html>${renderToStaticMarkup(/*#__PURE__*/React.createElement(Html,Object.assign({},bodyProps,{headComponents:headComponents,htmlAttributes:htmlAttributes,bodyAttributes:bodyAttributes,preBodyComponents:preBodyComponents,postBodyComponents:postBodyComponents,body:bodyHtml,path:pagePath})))}`;return{html,unsafeBuiltinsUsage:global.unsafeBuiltinUsage};}catch(e){e.unsafeBuiltinsUsage=global.unsafeBuiltinUsage;throw e;}}
})();
